...
struct Contact
{
	enum TE // connection server state
	{
		EMPTY,
		ACCEPT,
		CONTACT
	}    type;
	enum ST // service server state
	{
		WORK,
		ABORT,
		TIMEOUT,
	...
	SOCKET      s; // socket for delivering data to client
	SOCKADDR_IN prms; // socket params
	int         lprms; // sizeof(SOCKADDR_IN)
	HANDLE      hthread; // process/thread handle
	HANDLE      htimer; // timer handle
	HANDLE		serverHThtead;

	char msg[50];
	char srvname[15]; // name of service server
...
DWORD WINAPI TimerThread(LPVOID pPrm)
{
	HANDLE hTimer = CreateWaitableTimer(NULL, TRUE, NULL);
	if (!hTimer)
	{
		printf("Failed to create timer: %ld\n", GetLastError());
		return 1;
	}

	// Устанавливаем таймер на 3 минуты
	LARGE_INTEGER liDueTime;
	liDueTime.QuadPart = -1200000000LL; // 1 минута в 100-наносекундных интервалах (отрицательное значение для относительного времени)
	...
	printf("Timer set for 3 minute. Waiting...\n");

	if (WaitForSingleObject(hTimer, INFINITE) == WAIT_OBJECT_0)
	{
		printf("Timer expired. Stopping server...\n");
		for (auto& contact : contacts) {
			if (contact.type == Contact::ACCEPT || contact.type == Contact::CONTACT) {
				...
				if (sendResult == SOCKET_ERROR) {
					...
					printf("Sent TimeOUT to client %s\n", contact.srvname);
				}
			}
		}

		*((TalkersCommand*)pPrm) = SHUTDOWN;
	}
	else
	{
		printf("WaitForSingleObject failed: %ld\n", GetLastError());
	}
...
int main(int argc, char* argv[])
{
	...
	HANDLE hTimerThread = CreateThread(NULL, 0, TimerThread, (LPVOID)&cmd, 0, NULL);
	if (!hTimerThread)
	{
		printf("Failed to create timer thread: %ld\n", GetLastError());
		return 1;
	}
